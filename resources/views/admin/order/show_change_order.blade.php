...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <div align="right" style="margin-right: 40px;">
            ...
            @endforeach
            @endpermission
            <a href="{{  url('/admin/order/change') }}" class="btn btn-sm btn-danger">back</a>
        </div>
                ...
                @foreach($order as $orders)
                  ...
                  {{--      <td class="center">{{ $orders->item->gender->name}}</td>--}}
                        ...
                        @if($orders->item->statues_item_store==0)
                            <td class="center"><a href="{{ url('/admin/order/statues_item/'.$orders->item->id)}}" class="btn btn-sm btn-danger">not found</a></td>
                        @elseif($orders->item->statues_item_store==4)
                            <td class="center"><a href="{{ url('/admin/order/statues_item/'.$orders->item->id)}}" class="btn btn-sm btn-primary">found</a></td>
                        @endif
                    </tr>
                @endforeach
                </tbody>
